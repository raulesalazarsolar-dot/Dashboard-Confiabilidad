...
def generar_html_moderno(db_json):
    ...
    # Procesar imagen del Avatar si existe en el entorno
    avatar_base64 = ""
...
<div class="top-bar">
  <div class="brand">
    <img src="https://upload.wikimedia.org/wikipedia/commons/c/ca/Walmart_logo.svg" alt="Walmart" style="height:30px;filter:brightness(0) invert(1);">
...
<div class="main-container">
  <div class="sidebar">
    ...
    <div style="padding:20px; border-top:1px solid var(--border); display:flex; flex-direction:column; align-items:center; width:100%;">
      ...
      <!-- ESPACIO PARA TEAM BRANDING INTERNO (ARRIBA DEL BOTÓN EXCEL) -->
...
function renderResumen() {
  ...
  Object.keys(lnByPlanta).sort().forEach(planta => {
    ...
    let isCarnesArea = False;
    
    if (pKeyLowerCase.includes('carne') || pKeyLowerCase.includes('mercadeo') || pKeyLowerCase.includes('molida')) { 
        tema = 'tema-carnes'; 
    ...
    const numero_top = isCarnesArea ? 6 : 3;
    const titulo_top = `Top ${numero_top} Equipos Críticos`;

    const top_equipos = eqs.map(e => {
      const lnOp  = lineas[e.linea]?.op || 0;
      const mtbf  = e.det > 0 ? lnOp / e.det : 0;
      const mttr  = e.det > 0 ? e.hrs / e.det : 0;
      ...
      return { ...e, prob, mtbf, mttr, probPrev, mtbfPrev };
        ...
        return true;
    }).sort((a,b)=>b.prob-a.prob).slice(0, numero_top);
    ...
    const rankColors = ['#CC2222','#E65100','#F9A825', '#C0392B', '#D35400', '#E67E22'];
    const filas = top_equipos.map((e, i) => {
      ...
      return `
        <div class="grid-tr">
          ...
          <div class="stat-box bg-gray-light">
            <span class="stat-val" style="color:#7F8C8D">	ext{\${e.probPrev.toFixed(1)}}%</span>
          </div>
      ...
      <div class="fila-planta-v3">
        <div class="top-section">
          ...
          <div class="grid-table">
            ...
            \${top_equipos.length ? filas : '<p style="color:var(--text-muted);font-size:0.9rem;padding:15px 0;">Sin detenciones críticas en el período.</p>'}
          </div>
        </div>
