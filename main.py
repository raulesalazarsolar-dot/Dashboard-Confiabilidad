...
def buscar_tiempo_detencion_hr(df):
    for c in df.columns:
        ...
        if 'perdida' in cl and 'hr' in cl: return c
    # Fallback a minutos si no hay horas
    for c in df.columns:
        cl = str(c).lower().replace('  ', ' ').strip()
        if 'detencion' in cl and 'min' in cl: return c
    return None
...
def procesar_datos_confiabilidad():
    ...
    for archivo_nombre in archivos:
        ...
        try:
            ...
            is_min = 'min' in str(col_tpo_det).lower()
            factor = 60.0 if is_min else 1.0
            df_det['Hrs_Perdidas'] = pd.to_numeric(df_det[col_tpo_det], errors='coerce').fillna(0) / factor
            
            df_det['LN'] = df_det[col_linea_det].apply(limpiar_linea)
            df_det['Equipo_Clean'] = df_det[col_equipo].astype(str).str.strip().str.title()
            df_det['SN'] = pd.to_numeric(df_det[col_semana_det].astype(str).str.replace(r'[^\d]', '', regex=True), errors='coerce').fillna(-1).astype(int)
            df_det = df_det[df_det['SN'] > 0]

            df_det = df_det[~((df_det['LN'] == 'L2') & (df_det['SN'] < 15))]
            df_det = df_det[~((df_det['LN'] == 'L4') & (df_det['SN'] < 19))]

            agrup_det = df_det.groupby(['LN', 'SN']).agg(tpop=('Hrs_Perdidas', 'sum')).reset_index()
            ...
            if not all([col_semana_tpo, col_linea_tpo]):
                print(f"   ❌ Faltan columnas de tiempo/linea en {archivo_nombre}. Saltando...")
                continue

            df_tpo = df_tpo.dropna(subset=[col_linea_tpo, col_semana_tpo])
            df_tpo['LN'] = df_tpo[col_linea_tpo].apply(limpiar_linea)
            df_tpo['SN'] = pd.to_numeric(df_tpo[col_semana_tpo].astype(str).str.replace(r'[^\d]', '', regex=True), errors='coerce').fillna(-1).astype(int)
            df_tpo = df_tpo[df_tpo['SN'] > 0]

            if col_tpo_plan:
                df_tpo['Hrs_Plan'] = pd.to_numeric(df_tpo[col_tpo_plan], errors='coerce').fillna(0)
            elif col_tpo_oper:
                df_tpo['Hrs_Plan'] = pd.to_numeric(df_tpo[col_tpo_oper], errors='coerce').fillna(0)
            else:
                df_tpo['Hrs_Plan'] = 0

            df_tpo = df_tpo[~((df_tpo['LN'] == 'L2') & (df_tpo['SN'] < 15))]
            df_tpo = df_tpo[~((df_tpo['LN'] == 'L4') & (df_tpo['SN'] < 19))]
            ...
            for _, r in merged.iterrows():
                ...
                tpo_perdido_linea = float(r['tpop'])
                
                # Si se tomó tiempo operativo como base porque no existía planificado, reconstruimos el planificado real sumándole las detenciones
                if not col_tpo_plan and col_tpo_oper:
            ...
            for _, r in df_det.iterrows():
                datos_equipos.append({
                    ...
                    "planta": planta_nombre,
                    "linea": r['LN'],
                    "equipo": r['Equipo_Clean'],
                    "semana": int(r['SN']),
                    "detenciones": 1,
                    "tpo_perdido_eq": float(r['Hrs_Perdidas']),
                    "fecha": str(r.get('Fecha', 'N/A'))[:10] if 'Fecha' in df_det.columns else 'N/A',
                    "componente": str(r.get('Componente', 'N/A')) if 'Componente' in df_det.columns else 'N/A',
                    "tipo": str(r.get('Tipo Detención', 'N/A')) if 'Tipo Detención' in df_det.columns else 'N/A'
                })
