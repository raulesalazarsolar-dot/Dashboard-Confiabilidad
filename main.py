...
# ==========================================
# 2. FUNCIONES ETL NORMALIZADAS
# ==========================================
def limpiar_linea(nombre):
    if not isinstance(nombre, str): return "OTRO"
    n = nombre.upper().strip()
    if 'L1' in n or 'HALLULLA' in n: return 'L1'
    if 'L2' in n or 'MARRAQUETA' in n: return 'L2'
    if 'L3' in n or 'FONDO' in n: return 'L3'
    if 'L4' in n: return 'L4'
    if 'L5' in n: return 'L5'
    return n
...
def procesar_datos_confiabilidad():
    ...
    for archivo in os.listdir(DATA_DIR):
        if not archivo.endswith('.xlsx') or archivo.startswith('~'): continue
        try:
            xl = pd.ExcelFile(os.path.join(DATA_DIR, archivo))
            df_det = pd.read_excel(xl, sheet_name=[h for h in xl.sheet_names if 'Detenciones' in h][0])
            df_tpo = pd.read_excel(xl, sheet_name=[h for h in xl.sheet_names if 'Tiempos' in h][0])
            
            # Limpiar nombre de planta (Quita .xlsx y prefijo)
            planta_nombre = archivo.replace("Confiabilidad ", "").replace(".xlsx", "").strip()
            super_planta = "Carnes" if "carne" in planta_nombre.lower() else "Masas"
            
            # Normalización de columnas Detenciones
            c_linea = [c for c in df_det.columns if 'linea' in str(c).lower()][0]
            c_sem = [c for c in df_det.columns if 'semana' in str(c).lower()][0]
            
            df_det['LN'] = df_det[c_linea].apply(limpiar_linea)
            df_det['SN'] = pd.to_numeric(df_det[c_sem].astype(str).str.replace(r'[^\d]', '', regex=True), errors='coerce').fillna(0).astype(int)
            
            # Filtro estricto solicitado L2/L4
            df_det = df_det[~((df_det['LN'] == 'L2') & (df_det['SN'] < 15))]
            df_det = df_det[~((df_det['LN'] == 'L4') & (df_det['SN'] < 19))]
            
            # Agrupación detenciones
            agrup_det = df_det.groupby(['LN', 'SN']).agg(tpop=('Hrs detencion total [min]', 'sum'), det=('LN', 'count')).reset_index()
            
            # Normalización de columnas Tiempos
            df_tpo['LN'] = df_tpo[c_linea].apply(limpiar_linea)
            df_tpo['SN'] = pd.to_numeric(df_tpo[c_sem].astype(str).str.replace(r'[^\d]', '', regex=True), errors='coerce').fillna(0).astype(int)
            
            # Extraer Tiempos Planificados (o derivar de Operativo si Plan no existe)
            c_op = next((c for c in df_tpo.columns if 'operativo' in str(c).lower()), None)
            c_pl = next((c for c in df_tpo.columns if 'plan' in str(c).lower() or 'disponible' in str(c).lower()), None)
            
            if c_pl:
                df_tpo['Hrs_Plan'] = pd.to_numeric(df_tpo[c_pl], errors='coerce').fillna(0)
            elif c_op:
                df_tpo['Hrs_Plan'] = pd.to_numeric(df_tpo[c_op], errors='coerce').fillna(0)
            else:
                df_tpo['Hrs_Plan'] = 0
                
            agrup_tpo = df_tpo.groupby(['LN', 'SN']).agg(pl=('Hrs_Plan', 'sum')).reset_index()
            
            # Cruce de datos
            merged = pd.merge(agrup_tpo, agrup_det, on=['LN', 'SN'], how='outer').fillna(0)
            
            for _, r in merged.iterrows():
                plan_base = float(r['pl'])
                tpo_perdido_linea = float(r['tpop']) / 60.0
                
                # Si la fuente era Tiempo Operativo (porque no había Plan), el Planificado es Operativo + Perdido
                if not c_pl and c_op:
                    plan_base += tpo_perdido_linea
            ...
            for _, r in df_det.iterrows():
                datos_equipos.append({
                    ...
                    "planta": planta_nombre, 
                    "linea": r['LN'], 
                    "equipo": str(r['Equipo']).strip().title(), 
                    "semana": r['SN'], 
                    "detenciones": 1, 
                    "tpo_perdido_eq": float(r['Hrs detencion total [min]'])/60.0, 
                    "fecha": str(r.get('Fecha', 'N/A'))[:10], 
                    "componente": str(r.get('Componente', 'N/A')), 
                    "tipo": str(r.get('Tipo Detención', 'N/A'))
                })
...
# ==========================================
def generar_html(db_json):
    fecha_actual = datetime.now(ZoneInfo("America/Santiago")).strftime("%d/%m/%Y %H:%M")

    html = """<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8"><title>Dashboard Confiabilidad</title>
    ...
    function applyFilters() {
      ...
      tableDataFull = Object.values(eqMap).map(d => {
        ...
        // REQUERIMIENTO ESPECÍFICO APLICADO: Tpo Operativo = Planificado - Perdido
        let opTime = Math.max(0, planTime - d.tpop);
        
        // REQUERIMIENTO ESPECÍFICO APLICADO: MTBF usando el Tpo Operativo calculado
      ...
      // REQUERIMIENTO ESPECÍFICO APLICADO: KPI global = Planificado global - Perdido global
      let operativoGlobal = Math.max(0, totalPlanificadoGlobal - sumPerdidoGlobal);
      ...
      document.getElementById('k_oper').innerText        = operativoGlobal.toFixed(1);
    ...
    function drawCharts(sDesde, sHasta) {
      ...
      weeks.forEach(w => {
        ...
        let sOp = Math.max(0, sPlan - sPerd);
    ...
    print(f"\n✅ Dashboard generado correctamente en: {OUTPUT_HTML}")
